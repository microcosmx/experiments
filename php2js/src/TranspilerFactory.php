...
class TranspilerFactory
{
    ...
    public static function create(Node $node, $context)
    {
        ...
        if (!array_key_exists($type, self::$transpilersMap)) {
            $className = "\\Php2js\\Transpilers\\" . self::$transpilersMap['Scalar_String'] . 'Transpiler';
        ...
        //print $str;
        //print(serialize($context->node->name));
        //print(serialize($context->node->value));
        //print(serialize($context->node->expr));
